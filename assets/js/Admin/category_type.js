$(document).ready(function () {
 ...
 // category creation
 $(document).on("click", "#category_type_add_button", function (e) {
    e.preventDefault();

    var formData = $("#category_type_form").serialize();
    $.ajax({
      ...
      success: function (data) {
        if (data != "") {
          if (data.response == "success") {
            ...
            $("#category_type_model").modal("hide");
            $("#category_type-list").load(location.href + " #category_type-list>*", "");
            $("#card_id").load(location.href + " #card_id>*", "");
            $('#category_type_form')[0].reset();
        } else {
